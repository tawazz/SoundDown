package net.tawazz.sounddown.mp3agic;
...
public abstract class AbstractID3v2FrameData {
	...
	protected void synchroniseAndUnpackFrameData(byte[] bytes) throws net.tawazz.sounddown.mp3agic.InvalidDataException {
		if (unsynchronisation && BufferTools.sizeSynchronisationWouldSubtract(bytes) > 0) {
			byte[] synchronisedBytes = net.tawazz.sounddown.mp3agic.BufferTools.synchroniseBuffer(bytes);
